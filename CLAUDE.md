...
- Persistence enters as EFFECT CLOSURES, not a Store interface: `New(...Option)` returns a Manager holding only validated settings, and each method takes the effects it performs (aim for 2, distinctly named types, nil → `ErrNilEffect`); framework-constructed seams (middleware, supervisor loops) get a factory currying the effects (`mgr.Verifier(load, touch)`). No shipped memory store — tests state closures inline and never test a fake. See docs/design.md §Design DNA; `auth/apikey` is the reference package.
