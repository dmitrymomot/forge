...
func TestMemoryStore_NotFound(t *testing.T) {
	...
	_, err := s.Load(ctx, id.UUID{15: 9})
	assert.ErrorIs(t, err, apikey.ErrNotFound)
	_, err = s.LoadByHash(ctx, "missing")
	assert.ErrorIs(t, err, apikey.ErrNotFound)
...
func TestMemoryStore_Duplicate(t *testing.T) {
	...
	s := apikey.NewMemoryStore()
	ctx := context.Background()
	require.NoError(t, s.Save(ctx, mkKey(1, "h1", "u1", "")))

	assert.ErrorIs(t, s.Save(ctx, mkKey(1, "h-other", "u1", "")), apikey.ErrDuplicate) // same ID
	assert.ErrorIs(t, s.Save(ctx, mkKey(2, "h1", "u1", "")), apikey.ErrDuplicate)      // same hash
}
...
func TestMemoryStore_ListFilterAndOrder(t *testing.T) {
	...
	s := apikey.NewMemoryStore()
	ctx := context.Background()
	require.NoError(t, s.Save(ctx, mkKey(1, "h1", "u1", "t1")))
	require.NoError(t, s.Save(ctx, mkKey(2, "h2", "u2", "t1")))
	require.NoError(t, s.Save(ctx, mkKey(3, "h3", "u1", "t2")))
...
func TestMemoryStore_Swap(t *testing.T) {
	t.Parallel()
	s := apikey.NewMemoryStore()
	ctx := context.Background()
	old := mkKey(1, "h1", "u1", "")
	...
	require.NoError(t, s.Swap(ctx, old.ID, at, mkKey(2, "h2", "u1", "")))

	gotOld, err := s.Load(ctx, old.ID)
	require.NoError(t, err)
	assert.True(t, gotOld.ExpiresAt.Equal(at))
	_, err = s.LoadByHash(ctx, "h2")
	assert.NoError(t, err)
}

// TestMemoryStore_SwapRejectsUnknownOldKey pins that Swap writes nothing
// when the key it must expire is gone, so a rotation cannot leave an
// orphan replacement behind.
func TestMemoryStore_SwapRejectsUnknownOldKey(t *testing.T) {
	t.Parallel()
	s := apikey.NewMemoryStore()
	ctx := context.Background()

	err := s.Swap(ctx, id.UUID{15: 9}, time.Now().UTC(), mkKey(2, "h2", "u1", ""))
	assert.ErrorIs(t, err, apikey.ErrNotFound)
...
func TestMemoryStore_CloneIsolation(t *testing.T) {
	...
	k.Meta["k"] = "mutated"
	got1, err := s.Load(ctx, k.ID)
	require.NoError(t, err)
	assert.Equal(t, "v", got1.Meta["k"])

	got1.Meta["k"] = "mutated-again"
	got1.Scopes[0] = "write"
	got2, err := s.Load(ctx, k.ID)
	require.NoError(t, err)
	assert.Equal(t, "v", got2.Meta["k"])
	assert.Equal(t, "read", got2.Scopes[0])
}

// TestMemoryStore_NilScopesMetaNormalized pins the read shape every backend
...
// identical shapes whichever backend supplies the effects.
