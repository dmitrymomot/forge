...
func TestEnsureIndexes_Integration(t *testing.T) {
	db := openTestDB(t) // from lifecycle_integration_test.go
...
func TestSharding_Integration(t *testing.T) {
	// Sharding commands require a mongos (sharded cluster), which cannot be
	// provisioned by mongotest. Gate on a dedicated var.
