...
- `problem` — a pluggable error-**Responder** seam shipping four responders — JSON (RFC 9457), plain-text (`text/template`), HTML (caller's `html/template`), and Component (`render.Templ`/templ) — plus `Negotiate(fallback, byType)` that dispatches by `Accept`. forge authors no markup: JSON/Text render from Go, HTML/Component render the consumer's template/component.
...
```go
func JSON(opts ...Option) Responder                                            // application/problem+json
func Text(opts ...Option) Responder                                            // text/plain via text/template
func HTML(t *template.Template, name string, opts ...Option) Responder          // text/html, caller's html/template
func Component(build func(Problem) render.Component, opts ...Option) Responder  // text/html via render.Templ (templ)
func Negotiate(fallback Responder, byType map[string]Responder) Responder       // dispatch by Accept
```

- **No forge-authored markup.** `JSON` and `Text` render from Go (`Text` uses `text/template`, never `html/template`). `HTML` and `Component` render from the **consumer's** `html/template` / `render.Component`, so any markup lives in consumer code — forge ships no default page. `WithTemplate(*text/template.Template)` overrides only the `Text` layout; `HTML` takes its template positionally (`name` selects a `{{define}}` block, `""` runs `Execute`); `Component` takes a `func(Problem) render.Component` builder.
- `Negotiate(fallback, byType)` scans the `Accept` header in order; the first media range that **exactly** matches a key in `byType` wins, else `fallback` (typically `JSON`). Wildcards, unmatched types, or an absent `Accept` → `fallback`. Q-values are not weighted — full weighting is the standalone `negotiate` package's job later; this covers browser-vs-API (a browser leads with `text/html`, curl/API sends `*/*`/`application/json`).
- **All four responders share `(config).log5xx`:** a 5xx is logged (with `r.Context()`, so `request_id`/`client_ip` ride along) but `err.Error()` never enters the body — `From` omits `Detail` for 5xx, so every body shows only status/title.
...
- **problem** — `From` status/code/field mapping for `request.Error`, `validate.Errors`, coded and plain errors; `JSON` emits `application/problem+json`; `Text` emits `text/plain` (default layout + `WithTemplate`); `HTML` renders a caller `html/template`; `Component` renders a `render.Component`; `Negotiate` dispatches by `Accept` (mapped type wins, else fallback, absent → fallback); **no body contains `err.Error()` on 5xx**; `WithStatus` override; a custom `Responder` is honored where one is passed.
...
`timeout`, `bodylimit`, `cors`, `compress`, `bind`, standalone `negotiate`, `static`, `upload`, `proxy`, `conditional` — the next P3 PR. Full realclientip-style pluggable Strategy *interface* — not needed; the option-based strategies + presets cover the cases, and a rarely-needed one can be added as another option later. forge ships no default error **markup**: `problem` provides the `HTML` and `Component` responder *wrappers*, but the actual page is the consumer's `html/template` or `render.Component`. Rich-HTML/UGC sanitization stays out (would need a heavy dep); `html/template`/templ auto-escaping covers output.
