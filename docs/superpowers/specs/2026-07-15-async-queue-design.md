...
- **Multi-queue per-queue-budget or single-queue-per-Service topologies** — weighted single pool chosen (user decision) for one-Service ergonomics; strict mode covers hard priority.
