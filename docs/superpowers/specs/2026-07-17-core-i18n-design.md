...
- **`Key`** — a declared message-key handle (`var titleKey = i18n.NewKey("dashboard.title")` at package-var level). Its purpose is **typo-catching**: `Bundle.ValidateKeys(keys ...Key) error` checks a binary's declared keys against the default locale's catalog at startup, turning a silent key-echo in production into a construction error. Validation is explicit rather than a package-global registry: a global registry would break binaries constructing multiple bundles over different catalogs, and validation is the one thing that must not silently pass against the wrong catalog. `Key` is a plain comparable value; `TK`/`TNK` delegate to `T`/`TN`. Interning a key to an index — so `TK` skips the map lookup — is a **deferred optimization**, not part of the initial design: per design.md, perf-motivated complexity requires a benchmark proving it, so it lands (or does not) in the post-benchmark pass with before/after numbers.
...
- **Plural-form probing** (non-fatal, via the miss handler with `Reason: MissingForm`): for each plural message, `SupportedForms(rule)` is compared against the forms the catalog defines, and **both** directions are reported — a form the rule produces that the catalog lacks (incomplete translation), and a form the catalog defines that the rule can never produce (dead translation). Probing reports; it never fails construction. Three exclusions keep it signal, not noise:
  - **Only locales with an explicitly wired rule are probed.** The built-in default rule is not a claim about anyone's grammar, so there is nothing to check a catalog against; probing an English `{one, other}` catalog against the default's `{Zero, One, Many}` output would report three findings that are all correct-as-written.
  - **`zero` is never reported as dead** — the zero convention means a catalog may legitimately define a `zero` form the rule never produces.
  - **`other` is never reported as dead** — it is the terminal form of every fallback chain, so defining it is always a valid safety net even where the rule cannot produce it.
...
- `T` costs one map lookup for the message key, over a per-locale map of precompiled messages. That is the readable design and the one to build first. Replacing it with a bundle-wide key index and per-locale slices — which would also make `TK` an array index — is a candidate for the post-benchmark pass, gated on the benchmark showing the lookup is material (design.md: "optimize only what a benchmark or profile proves hot, and any perf-motivated complexity requires a benchmark in the PR proving it").
