...
- Path `core/i18n`, package `i18n`. Deps: `core/money`, `ops/logger` (nope-logger default). No x/text, no YAML, no external deps.
...
bundle, err := i18n.New(
    ...
    i18n.WithMissingKeyLogger(log),             // *slog.Logger, default logger.NewNope()
)
```
...
- **Validation at `New`**: unparseable JSON, empty catalogs, unknown default, registered `Key`s missing from the default catalog, and plural-form probing — for each language, warn via the logger when a message defines forms its rule never produces or misses forms it does (port of `SupportedPluralForms`).
...
- **Language fallback** per lookup: exact locale → base language (`en-US`→`en`) → default locale → key echo (+ missing-key log).
...
- `LocaleExtractor` — a `logger.ContextExtractor` adding the resolved locale to log attrs.
...
- Runtime never returns errors: missing key → key echo + missing-key log; missing localizer in ctx → defaults; zero `Locale` → default. Rendering is total.
