...
// CSV streams records as "text/csv; charset=utf-8" with the given status code (unless
// the caller has already set a Content-Type). When filename is non-empty a
// Content-Disposition: attachment header is set with an RFC 8187-safe filename. CSV
// streams (it is not buffered), so a write error mid-output may leave a partial body;
